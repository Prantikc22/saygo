fn main() {
  ...
  if std::env::var_os("SAYGO_CROSS_BUILD").is_some() {
    ...
    for file in ["acl-manifests.json", "__global-api-script.js"] {
      std::fs::copy(native_out.join(file), out_dir.join(file)).expect("native Tauri build metadata is copied");
    }
    std::fs::create_dir_all(out_dir.join("app-manifest")).expect("app manifest directory");
    std::fs::write(out_dir.join("app-manifest/__app__-permission-files"), "[]").expect("app manifest");
    return;
  }
  tauri_build::build()
}
